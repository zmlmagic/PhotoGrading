...
@interface PGLoginViewController()<UITableViewDataSource,UITableViewDelegate,TencentSessionDelegate,PGWeiBoDelegate,
...
{
    [super viewDidLoad];
    self.view.backgroundColor = RGB(223, 51, 28);
...
- (void)refreshSelfView
{
    ...
    if (![PGUserInfo isLogin]) {
        ...
        button_logout.frame = CGRectMake(320 - 32, 30, 22, 22);
                                                     ...
                                                     completionHandler:^(NSDictionary *jsonDic)
                        {
                            ...
                            if (model &&[model.result intValue] == 0) {
                                ...
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    ...
                                    [[NSNotificationCenter defaultCenter] postNotificationName:notification_loginRight object:_string_sign];
                                    [self back:nil];
                                });
                                ...
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    [self presentViewController:inputInfoVC animated:YES completion:^{}];
                                });
                            }
                        }
